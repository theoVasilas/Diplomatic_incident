...
def get_fit_weighted_average(cfg, ):
    ...
    def weighted_average(metrics: List[Tuple[int, Metrics]]) -> Metrics:
        ...
        clients_elapsed_times = [num_examples * m["elapsed_time"] for num_examples, m in metrics]
        clients_avg_time = sum(clients_elapsed_times) / sum(examples)


        all_max_norms = [m["max_norm_before_clipping"] for _, m in metrics] 
        max_norm = max(all_max_norms)

        grand_70_pecs = [m["grand_70_pec"] for _, m in metrics]
        grand_70_pec = min(grand_70_pecs)
        ...
        Aggregations_entry = (
            ...
            "max_norm" : max_norm,
            "min_grand_70_pec" : grand_70_pec,
            "clients_avg_time":clients_avg_time,
            },
            # grand_70_pecs,
        )
        ...
        print(f"\t fit_clients_aggregation - noise_multiplier: {noise_multiplier} epsilon: {epsilon} ")
        print(f"\t for target_delta {cfg.target_delta} max_grad_norm {cfg.max_grad_norm}")
        print(f"\t loss: {loss} \n")

        # cfg.max_grad_norm = grand_70_pec # adaptive clipping
        # print(f"\t\t test change hydra {cfg.max_grad_norm}") 
...
def run_my_sim(cfg, ):
    ...
    # backend_config = {"client_resources": {"num_cpus": cfg.num_cpus, "num_gpus": cfg.num_gpus}}
